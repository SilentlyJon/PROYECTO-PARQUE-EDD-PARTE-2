...
struct Parque {
    int cap_max;
    struct NodoVisitantes *raiz_visitantes; /* arbol binario ordenado por id del visitante */
...
void ver_reporte_general_dia(struct Parque *parque) {
    ...
    if (mas_visitada == NULL || mayor_pico == NULL || mayor_pico->cap_max == 0) {
        printf("[ERROR]: No se puede generar el reporte (Datos insuficientes o invalidos en las atracciones).\n");
        printf("\nPresione ENTER para regresar al menu principal...");
        while (getchar() != '\n');
        return;
    }
...
void ver_reporte_atracciones_actual(struct NodoZonas *head_zonas) {
    ...
    if (num_top == 0) {
        printf("No hay atracciones registradas o con datos validos en el parque.\n");
        printf("\nPresione ENTER para regresar al menu principal...");
        while (getchar() != '\n');
        return;
    }
    ...
    printf("\n=================================================================================\n");
    printf("                      RANKING TOP 10 ATRACCIONES MAS VISITADAS                   \n");
    printf("=================================================================================\n");
...
void ver_entradas_compradas(struct NodoEntradas *head_entradas) {
    ...
    while (actual != NULL) {
        if (actual->entrada != NULL) {
            ...
            /* Se imprime la informacion alineada incluyendo el estado */
...
int avanzar_fila_atraccion(struct NodoZonas *head_zonas, int id_atraccion) {
    ...
    while (cap_disponible > 0 && (!p_bloqueada || !g_bloqueada)) {
        ...
        if (fila_actual->frente->tam_grupo <= cap_disponible) {
            ...
            /* CAMBIO: Solo alternar si la otra cola esta operativa */
            if (!*bloqueada_otra) {
                turno_prioritaria = !turno_prioritaria;
            }
        }

        /* CASO B: El grupo del frente excede la capacidad. Buscamos atras. */
        else {
...
int agregar_visitante(struct Parque *parque, struct Entrada *entrada, char *nombre, char *rut, int edad, float altura) {
    ...
    if (contar_dentro_parque(parque->raiz_visitantes) + 1 > parque->cap_max){
        return -3; /* Capacidad maxima de parque alcanzada */
    }
...
int menu_inicializar_parque(struct Parque *parque) {
    ...
    printf(
        ...
        "=========================================================================\n"
        "     Para empezar a manejar el sistema el dia de hoy, debe ingresar\n"
        "          los siguientes parametros en una sola linea y en orden.\n"
        "                  Escriba '0' para cerrar el programa\n"
        "=========================================================================\n\n"
    );
    printf(
        " - Los parametros son (separados por espacios):\n"
        "[1] Capacidad Maxima de Visitantes del Parque \n"
    ...
    while (!es_valido) {
        ...
        if (asignados <= 0) {
            printf("[ERROR] No se detecto ningun ingreso valido. Intente de nuevo.\n\n");
            continue;
        }
        ...
        if (cap_max_buf <= 0 || general_buf < 0 || infantil_buf < 0 || familiar_buf < 0 || prioritaria_buf < 0) {
            printf("[ERROR] Valores invalidos. La capacidad debe ser positiva y los precios no pueden ser negativos.\n\n");
            continue;
        }
    ...
    if (parque != NULL) {
        ...
        printf("[SISTEMA] !Parque inicializado con exito!\n");
    } else {
...
void menu_comprar_entrada(struct NodoEntradas **entradas) {
    ...
    printf(
        "- INFORMACION\n"
        "[1] Los parametros son: Tipo de Entrada y Cantidad a Comprar\n"
        ...
        "    tres miembros por el precio de 1 entrada. Por lo que la cantidad maxima\n"
    ...
    while (!es_valido) {
        ...
        if (asignados <= 0) {
            printf("[ERROR] No se detecto ninguna entrada valida. Intente de nuevo.\n\n");
            continue;
        }

        if (asignados == 1) {
            printf("[ERROR] Falta el segundo parametro. Debe ingresar la cantidad despues del tipo.\n\n");
            continue;
        }

        if (asignados > 2) {
            printf("[ERROR] Entrada invalida. Escribio demasiados parametros (maximo 2).\n\n");
            continue;
        }
        ...
        if (strcmp(tipo_buf, "general") != 0 &&
            ...
            printf("[INFO] Tipos permitidos estrictamente en minusculas: [general, infantil, familiar, prioritaria]\n\n");
            continue;
        }

        if (cantidad_buf <= 0) {
            printf("[ERROR] La cantidad (%d) no es valida. Debe ser mayor a cero.\n\n", cantidad_buf);
            continue;
        }

        if (strcmp(tipo_buf, "familiar") == 0 && cantidad_buf > 4) {
            printf("[ERROR] La cantidad maxima permitida para entradas de tipo 'familiar' es de 4.\n\n");
            continue;
        }
...
void menu_eliminar_entrada(struct NodoEntradas **entradas) {
    ...
    printf(
        ...
        "                           ++ ELIMINAR ENTRADA ++\n"
        "=========================================================================\n"
        " Para eliminar una entrada del parque, rellene los siguientes parametros \n"
    ...
    printf(
        ...
        "- INFORMACION\n"
        "[1] El parametro es unicamente la ID de la entrada\n"
        "Por ejemplo: '345'\n\n"
...
void menu_cambiar_estado_entrada(struct NodoEntradas **entradas) {
    ...
    printf(
        ...
        "=========================================================================\n"
        " Para cambiar el estado de la entrada, rellene los siguientes parametros \n"
        "                         en el orden que se indica.\n"
        "                 Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
        "- INFORMACION\n"
        "[1] Los parametros son: ID de la entrada, y el nuevo estado\n"
        "[2] Deben escribirse en conjunto y separados por un espacio\n"
    ...
    while (!es_valido) {
        ...
        if (asignados <= 0) {
            printf("[ERROR] Formato incorrecto. El primer parametro debe ser el ID numerico de la entrada.\n\n");
            continue;
        }

        if (asignados == 1) {
            printf("[ERROR] Falta el segundo parametro. Debe ingresar el nuevo estado despues del ID.\n\n");
            continue;
        }

        if (asignados > 2) {
            printf("[ERROR] Entrada invalida. Escribio parametros de mas (maximo 2).\n\n");
            continue;
        }
    ...
    if (cambiar_estado_entrada(entradas, id_buf, estado_buf)) {
        printf("[SISTEMA] Estado de la entrada %d actualizado a '%s' con exito\n", id_buf, estado_buf);
    } else {
        printf("[ERROR] No se encontro ninguna entrada con el ID %d o no se pudo modificar.\n", id_buf);
    }
...
void menu_agregar_visitante(struct Parque *parque) {
    ...
    printf(
        ...
        "                          ++ AGREGAR VISITANTE ++\n"
        "=========================================================================\n"
        "      Para agregar a un visitante, rellene los siguientes parametros \n"
        "                         en el orden que se indica.\n"
        "                 Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
        "- INFORMACION\n"
        "[1] Los parametros son: Nombre, RUT, Edad, Altura (mts) y la ID de Entrada\n"
        "[2] Deben escribirse en conjunto y separados por un espacio\n"
    ...
    while (!es_valido) {
        ...
        if (edad_buf < 0 || edad_buf > 120 || altura_buf <= 0.40f || altura_buf > 2.50f || id_entrada_buf <= 0) {
            printf("[ERROR] Valores numericos incoherentes. Verifique edad, altura e ID de entrada.\n\n");
            continue;
        }

        entrada_visitante = buscar_entrada_por_id(parque->head_entradas, id_entrada_buf);
        if (entrada_visitante == NULL) {
            printf("[ERROR] La entrada con ID %d no existe o no esta registrada en el sistema.\n\n", id_entrada_buf);
            continue;
        }
    ...
    switch (resultado_logico) {
        ...
        case -3:
            printf("[ERROR] El Parque llego a su limite maximo.\n");
            break;
        case -4:
            printf("[ERROR] La entrada ya esta en uso.");
            break;
        default:
            printf("[ALERTA] Operacion rechazada. Codigo de error no catalogado (%d).\n", resultado_logico);
            break;
    }
...
void menu_eliminar_visitante(struct NodoVisitantes **raiz_visitantes) {
    ...
    printf(
        ...
        "                        ++ ELIMINAR VISITANTE ++\n"
        "=========================================================================\n"
        "      Para eliminar a un visitante, rellene los siguientes parametros \n"
        "                         en el orden que se indica.\n"
        "                 Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
        "- INFORMACION\n"
        "[1] El parametro es unicamente la ID del visitante\n"
        "[2] La eliminacion es permanente\n"
        "Por ejemplo: '67'\n\n"
    ...
    switch (resultado_logico) {
        ...
        case -1:
            printf("[ERROR] El arbol de visitantes esta vacio o no ha sido inicializado (NULL).\n");
            break;
        ...
        case -3:
            printf("[ERROR] Estructura corrupta: Se detecto una anomalia critica en la integridad del arbol.\n");
            break;

        default:
            printf("[ALERTA] Operacion rechazada. Codigo de error desconocido (%d).\n", resultado_logico);
            break;
    }
...
void menu_agregar_grupo_fila(struct Parque *parque) {
    ...
    printf(
        ...
        "=========================================================================\n"
        " Para agregar un grupo a la atraccion, rellene los siguientes parametros\n"
        "                         en el orden que se indica.\n"
        "                 Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
        "- INFORMACION\n"
        "[1] Los parametros son: IDs del Grupo, Tipo de Fila, ID de Atraccion\n"
        "[2] Maximo 10 IDs de integrantes pueden estar en el grupo.\n"
        "[3] Los tipo de fila pueden ser 'general' o 'prioritaria'\n"
        "[4] Todos los parametros deben estar separados por un espacio\n"
    ...
    while (!es_valido) {
        ...
        if (tam_grupo == 0) {
            printf("[ERROR] No se detecto ninguna ID de visitante al inicio. Intente de nuevo.\n\n");
            continue;
        }

        if (tam_grupo == 10) {
            if (sscanf(ptr_linea, "%d", &id_extra) == 1) {
                printf("[ERROR] Excedio el limite. El grupo puede contener un maximo de 10 visitantes.\n\n");
                continue;
            }
        }
        ...
        if (asignados == 1) {
            printf("[ERROR] Falta el ultimo parametro: Ingrese el ID de la atraccion de destino.\n\n");
            continue;
        }
        ...
        if (strcmp(tipo_buf, "general") != 0 && strcmp(tipo_buf, "prioritaria") != 0) {
            ...
            printf("[INFO] Debe escribir exactamente 'general' o 'prioritaria' en minusculas.\n\n");
            continue;
        }

        if (id_atraccion <= 0) {
            printf("[ERROR] El ID de la atraccion debe ser un numero entero positivo.\n\n");
            continue;
        }
    ...
    switch (resultado_logico) {
        ...
        case -1:
            printf("[ERROR] Operacion rechazada: La atraccion seleccionada no se encuentra operativa actualmente.\n");
            break;

        case -2:
            printf("[ERROR] Validacion fallida: Una o mas IDs ingresadas no corresponden a visitantes registrados.\n");
            break;

        case -3:
            printf("[ERROR] Restriccion fisica: Uno o mas integrantes del grupo no cumplen con la edad o altura minima.\n");
            break;

        case -4:
            printf("[ERROR] Acceso denegado: Algun integrante no posee una entrada prioritaria cuando el canal lo requiere.\n");
            break;

        case -5:
            printf("[ERROR] Error critico: No hay suficiente espacio en la memoria dinamica del sistema.\n");
            break;

        case -10:
            printf("[ERROR] Tamano invalido: La cantidad de integrantes proporcionada no es permitida para este grupo.\n");
            break;
...
void menu_avanzar_fila_atraccion(struct NodoZonas *head_zonas) {
    ...
    printf(
        ...
        "=========================================================================\n"
        "           Para avanzar las filas, rellene los siguientes parametros \n"
        "                         en el orden que se indica.\n"
        "                 Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
        "- INFORMACION\n"
        "[1] El unico parametro es la ID de la atraccion\n"
        "[2] Esto avanzara ambas filas dependiendo de su capacidad\n"
        "Por ejemplo:'23'\n\n"
    ...
    while (!es_valido) {
        ...
        if (atr_encontrada == NULL) {
            printf("[ERROR] No encontrada: No se encontro ninguna atraccion con el ID %d.\n\n", id_atraccion_buf);
            continue;
        }
    ...
    switch (resultado_logico) {
        case 0:
            printf("[SISTEMA] !Filas de la atraccion %d avanzadas con exito!\n", id_atraccion_buf);
            break;

        case -1:
            printf("[ERROR] No encontrada: La atraccion con ID %d no pudo ser localizada en el sistema de filas.\n", id_atraccion_buf);
            break;

        case -2:
            printf("[ERROR] Operacion omitida: Ambas filas (general y prioritaria) de la atraccion %d estan vacias.\n", id_atraccion_buf);
            break;
...
void menu_agregar_zona_al_parque(struct NodoZonas **head_zonas) {
    ...
    printf(
        ...
        "                            ++ AGREGAR ZONA ++\n"
        "=========================================================================\n"
        "         Para agregar una zona, rellene los siguientes parametros \n"
        "                         en el orden que se indica.\n"
        "                 Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
        "- INFORMACION\n"
        "[1] Los parametros son: Nombre, Tematica, Capacidad, Hora Apertura,\n"
        "Hora Cierre, y Maximo de Atracciones\n"
        "[2] Deben estar separados por un espacio y los nombres juntos.\n"
        "[3] Los tiempos estan en formato de 24 hrs.\n"
    ...
    while (!es_valido) {
        ...
        if (asignados < 8) {
            ...
            printf("[INFO] Se interpretaron con exito solo %d de los sub-parametros.\n\n", asignados);
            continue;
        }
        ...
        if (cap_max_buf <= 0 || max_atr_buf <= 0) {
            printf("[ERROR] La capacidad maxima y el maximo de atracciones deben ser numeros positivos.\n\n");
            continue;
        }

        if (h_ap_buf < 0 || h_ap_buf > 23 || m_ap_buf < 0 || m_ap_buf > 59) {
            printf("[ERROR] Hora de apertura invalida (%d:%02d). Ingrese un tiempo valido entre 00:00 y 23:59.\n\n", h_ap_buf, m_ap_buf);
            continue;
        }

        if (h_ci_buf < 0 || h_ci_buf > 23 || m_ci_buf < 0 || m_ci_buf > 59) {
            printf("[ERROR] Hora de cierre invalida (%d:%02d). Ingrese un tiempo valido entre 00:00 y 23:59.\n\n", h_ci_buf, m_ci_buf);
            continue;
        }
    ...
    switch (resultado_logico) {
        case 0:
            printf("[SISTEMA] !Zona '%s' de tematica '%s' agregada con exito al parque!\n", nombre_buf, tematica_buf);
            break;

        case -1:
            printf("[ERROR] Raiz nula: La estructura de control (head_zonas) no esta inicializada.\n");
            break;

        case -2:
            printf("[ERROR] Error de memoria: No hay suficiente espacio dinamico en el sistema.\n");
            break;
...
void menu_eliminar_zona_del_parque(struct NodoZonas **head_zonas) {
    ...
    printf(
        ...
        "                             ++ ELIMINAR ZONA ++\n"
        "=========================================================================\n"
        "       Para eliminar una zona, rellene los siguientes parametros \n"
        "                        en el orden que se indica.\n"
        "                 Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
        "- INFORMACION\n"
        "[1] El unico parametro es la ID de la Zona\n"
        "[2] Esto eliminara todas las atracciones dentro de la Zona\n"
        "Por ejemplo: '21'\n\n"
    ...
    switch (resultado_logico) {
        case 0:
            printf("[SISTEMA] Zona con ID %d y todas sus atracciones asociadas eliminadas con exito.\n", id_zona_buf);
            break;

        case -1:
            printf("[ERROR] Lista vacia: No existen zonas registradas en el parque actualmente.\n");
            break;

        case -2:
            printf("[ERROR] No encontrada: No se encontro ninguna zona con el ID %d en el sistema.\n", id_zona_buf);
            break;
...
void menu_agregar_o_remover_visitante_zona(struct NodoZonas *head_zonas) {
    ...
    printf(
        ...
        "=========================================================================\n"
        " Para agregar o remover a un visitante, rellene los siguientes parametros\n"
        "                        en el orden que se indica.\n"
        "                 Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
        "- INFORMACION\n"
        "[1] Los parametros son: Accion, Cantidad, ID de la Zona.\n"
    ...
    while (!es_valido) {
        ...
        if (asignados < 3) {
            printf("[ERROR] Datos incompletos. Debe ingresar la accion (agregar/remover), la cantidad y la ID de la zona.\n\n");
            continue;
        }
        ...
        if (strcmp(accion_buf, "agregar") != 0 && strcmp(accion_buf, "remover") != 0) {
            printf("[ERROR] Accion '%s' no valida.\n", accion_buf);
            printf("[INFO] Use estrictamente las palabras 'agregar' o 'remover' en minusculas.\n\n");
            continue;
        }

        if (cantidad_buf <= 0) {
            printf("[ERROR] La cantidad de visitantes debe ser un numero entero positivo.\n\n");
            continue;
        }

        if (id_zona_buf <= 0) {
            printf("[ERROR] ID de la zona invalido. El identificador debe ser un numero entero positivo.\n\n");
            continue;
        }
    ...
    switch (resultado_logico) {
        case 0:
            if (visitantes_param > 0) {
                printf("[SISTEMA] Se agregaron %d visitantes con exito a la zona ID %d.\n", cantidad_buf, id_zona_buf);
            } else {
                printf("[SISTEMA] Se removieron %d visitantes con exito de la zona ID %d.\n", cantidad_buf, id_zona_buf);
            }
            break;
        ...
        case -3:
            printf("[ERROR] Capacidad excedida: El ingreso de %d visitantes supera la capacidad maxima de la zona.\n", cantidad_buf);
            break;

        case -4:
            printf("[ERROR] Supera cantidad minima: No se pueden remover %d visitantes porque el aforo quedaria menor a cero.\n", cantidad_buf);
            break;
        case -5:
            printf("[ERROR] La Zona ya esta cerrada.\n");
            break;
        default:
            printf("[ALERTA] Operacion rechazada. Codigo de error desconocido (%d).\n", resultado_logico);
            break;
...
void mostrar_submenu_atracciones() {
    ...
    printf(
        ...
        "=========================================================================\n\n");
    printf(
        "[1] Agregar Atraccion a Zona\n"
        "[2] Eliminar Atraccion de Zona\n"
        "[3] Mover Atraccion a Distinta Zona\n"
        "[4] Modificar Estado de Atraccion\n\n"
...
void menu_agregar_atraccion_zona(struct NodoZonas *head_zonas) {
    ...
    printf(
        "=========================================================================\n"
        "                     ++ AGREGAR ATRACCION A ZONA ++\n"
        "=========================================================================\n"
        "       Para agregar una atraccion, rellene los siguientes parametros \n"
        "                         en el orden que se indica.\n"
        "                 Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
        "- INFORMACION\n"
        "[1] Los parametros son: ID Zona, Nombre, Tematica, Duracion (minutos), \n"
        "    Capacidad por Ciclo, Edad Minima, y Altura Minima (metros).\n"
...
void menu_eliminar_atraccion_zona(struct NodoZonas *head_zonas) {
    ...
    printf(
        "=========================================================================\n"
        "                    ++ ELIMINAR ATRACCION DE ZONA ++\n"
        "=========================================================================\n"
        "     Para eliminar una atraccion, rellene los siguientes parametros \n"
        "                         en el orden que se indica.\n"
        "                Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
    ...
    switch (resultado_logico) {
        case 0:
            printf("[SISTEMA] Atraccion con ID %d eliminada exitosamente del parque.\n", id_atraccion_buf);
            break;
        ...
        case -2:
            printf("[ERROR] No se pudo eliminar: No se encontro ninguna atraccion con el ID %d.\n", id_atraccion_buf);
            break;
...
void menu_mover_atraccion_distinta_zona(struct NodoZonas *head_zonas) {
    ...
    printf(
        ...
        "=========================================================================\n"
        "       Para mover una atraccion, rellene los siguientes parametros \n"
        "                        en el orden que se indica.\n"
        "                 Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
        "- INFORMACION\n"
        "[1] Los parametros son: ID Zona objetivo, ID Atraccion.\n"
    ...
    while (!es_valido) {
        ...
        if (asignados < 2) {
            printf("[ERROR] Datos incompletos. Debe ingresar la ID de la zona objetivo y la ID de la atraccion.\n\n");
            continue;
        }
        ...
        if (id_objetivo_buf <= 0 || id_atraccion_buf <= 0) {
            printf("[ERROR] IDs invalidas. Los identificadores deben ser numeros enteros positivos.\n\n");
            continue;
        }
    ...
    switch (resultado_logico) {
        case 0:
            printf("[SISTEMA] !Atraccion con ID %d movida con exito a la zona objetivo %d!\n",
                   id_atraccion_buf, id_objetivo_buf);
            break;

        case -2:
            printf("[ERROR] Incompatibilidad: La tematica de la atraccion no coincide con la identidad de la zona objetivo.\n");
            break;

        case -3:
            printf("[ERROR] Capacidad excedida: La zona objetivo con ID %d llego al maximo de su capacidad.\n", id_objetivo_buf);
            break;

        case -4:
            printf("[ERROR] No encontrada: No se encontro ninguna atraccion con el ID %d en ninguna zona del parque.\n", id_atraccion_buf);
            break;
...
void menu_modificar_estado_atraccion(struct NodoZonas *head_zonas) {
    ...
    printf(
        ...
        "=========================================================================\n"
        "     Para modificar una atraccion, rellene los siguientes parametros \n"
        "                        en el orden que se indica.\n"
        "                 Para volver atras y cancelar escriba 'volver'\n"
        "=========================================================================\n\n");
    printf(
        "- INFORMACION\n"
        "[1] Los parametros son: ID de Atraccion, y Nuevo Estado.\n"
        "[2] Los estados posibles son: 'operativa', 'mantenimiento', 'cerrada' y,\n"
        "'fuera_de_servicio'.\n"
        "[3] Todo debe estar en minusculas y sin espacios.\n"
    ...
    while (!es_valido) {
        ...
        if (asignados <= 0) {
            printf("[ERROR] Formato incorrecto. El primer parametro debe ser el ID numerico de la atraccion.\n\n");
            continue;
        }

        if (asignados == 1) {
            printf("[ERROR] Datos incompletos. Debe ingresar el nuevo estado despues del ID.\n\n");
            continue;
        }
    ...
    switch (resultado_logico) {
        case 0:
            printf("[SISTEMA] Estado de la atraccion %d actualizado a '%s' con exito\n",
        ...
        case -2:
            printf("[ERROR] No encontrada: No se encontro ninguna atraccion con el ID %d en el parque.\n", id_atraccion_buf);
            break;

        case -3:
            printf("[ERROR] Error critico: Falta memoria en el sistema para procesar el cambio.\n");
            break;
    ...
    while (continuar_programa) {
        ...
        switch (opcion_principal) {
            ...
            case 0:
                ...
                break;
            default:
                printf("\n[AVISO] Opcion invalida. Ingrese un numero entre 0 y 12.\n");
                printf("Presione ENTER para continuar...");
                while (getchar() != '\n');
        }
    }
